...
class DownloadCoinExternalWithdrawals implements ShouldQueue
{
    ...
    public function handle(Marketable $markets)
    {
        ...
        if (class_exists($service_name)) {
            ...
            if (method_exists($service, 'downloadExternalWithdrawals')) {
                $service->downloadExternalWithdrawals($this->coinSymbol);
            }

            Loggy::info('exchange', json_encode(['message' => 'External download withdrawal run on '.Carbon::now()->format('Y-m-d H:i:s')]));
        }
    }
}
